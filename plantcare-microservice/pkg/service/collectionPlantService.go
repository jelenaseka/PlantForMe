...
func (this *collectionPlantService) GetAllByCollectionId(collectionId uuid.UUID) ([]dto.CollectionPlantResponse, error_utils.MessageErr) {
	...
	for _, v := range collectionPlants {
		collectionPlantsResponse = append(collectionPlantsResponse, *dto.NewCollectionPlantResponse(v.ID, v.CollectionID, v.Nickname, v.ReferentPlantID, v.Base64Image, v.Tasks))
	}
...
func (this *collectionPlantService) GetOneById(id uuid.UUID) (*dto.CollectionPlantResponse, error_utils.MessageErr) {
	...
	collectionPlantResponse := dto.NewCollectionPlantResponse(collectionPlant.ID, collectionPlant.CollectionID, collectionPlant.Nickname, collectionPlant.ReferentPlantID, collectionPlant.Base64Image, collectionPlant.Tasks)
...
func (this *collectionPlantService) Create(collectionPlantRequest *dto.CollectionPlantRequest, username string) (*uuid.NullUUID, error_utils.MessageErr) {
	id := uuid.New()
	collectionPlant := data.NewCollectionPlant(id, collectionPlantRequest.CollectionID, collectionPlantRequest.Nickname, collectionPlantRequest.ReferentPlantID, collectionPlantRequest.Base64Image)
